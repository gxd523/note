...
> Synchronize内的代码不具有原子性，例如wait()、notify()
...
##### 生产者消费者模式
##### 交替加1
##### 为什么wait()、notify()、notifyAll()必须要在同步代码块中执行
> 主要是为了防止永久等待或者死锁的发生，比如进入同步代码块，执行wait()之前不可能在另一个同步代码块执行notify()。sleep()不需要像wait()、notify()这样要一个线程wait，一个线程去notify，只要设置当前线程睡眠，并且不会永久等待，所以不需要同步代码块保护。

##### 为什么wait()、notify()、notifyAll()定义在Object里面
> 这几个方法的对象与锁对象是同一个，而锁可以是任意对象。并且一个线程可以持有多个锁，所以这些方法不能设置为Thread类的方法。
