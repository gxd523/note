...
* `非阻塞`：即不阻塞线程，就是通过切线程执行耗时函数，不阻塞当前线程
* `非阻塞式挂起`：用同步方式(返回值)写异步逻辑(回调)，避免回调地狱(消除嵌套)
* 协程实现了在连续的代码块中切换线程，即实现了同步方式且不阻塞的切换线程
* `suspend`关键字的作用：函数创建者`提醒`函数调用者，这是一个耗时函数
...
#### 拦截器
![](https://raw.githubusercontent.com/gxd523/PictureBed/master/coroutineIntercepter.png)
...
* 挂起(suspend)：将挂起的任务根据调度器放到线程池中执行，继续执行
